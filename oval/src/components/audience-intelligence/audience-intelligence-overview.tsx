...
import { ArrowDown, ArrowUpRight, Bot, ChevronLeft, ChevronRight, Send, Sparkles, X } from "lucide-react";
...
const overviewSources = ["playstore", "freshdesk", "linkedin", "x", "reddit", "youtube"] as const;
const sourceEndpoint = (source: (typeof overviewSources)[number]) => `/api/${source}`;
...
export function AudienceIntelligenceOverview() {
  ...
  const openSource = (source: string) => source === "youtube" ? openPwYtVerse() : router.replace(`/audience-intelligence/${source}`);
  ...
  const model = useMemo(() => {
    ...
    const reddit = feeds.reddit || {}; const redditStats = reddit.stats || {};
    ...
    const cards: ChannelCard[] = [
      ...
      { id: "reddit", label: "Reddit", icon: SOURCES.reddit.icon, value: n(redditStats.totalMentions), unit: "posts captured", summary: `${fmt(n(redditStats.negativeCount))} negative and ${fmt(n(redditStats.positiveCount))} positive community signals across ${redditStats.topSubreddit || "Reddit"}.`, readings: [{ label: "Positive", value: n(redditStats.positiveCount), tone: "positive" }, { label: "Neutral", value: n(redditStats.neutralCount), tone: "neutral" }, { label: "Negative", value: n(redditStats.negativeCount), tone: "negative" }] },
    ...
    const redditLead = reddit.posts?.[0];
    ...
    if (redditLead?.title || redditLead?.snippet) insights.push({ id: "reddit", label: "Reddit", icon: SOURCES.reddit.icon, text: brief(redditLead.snippet || redditLead.title, "", 245), author: redditLead.subreddit ? `r/${redditLead.subreddit}` : "Reddit community", meta: `${redditLead.sentiment || "Public"} discussion${redditLead.createdAt ? ` · ${new Date(redditLead.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}` : ""}`, href: redditLead.url || "/audience-intelligence/reddit", external: Boolean(redditLead.url) });
    ...
    const signals: Signal[] = [
      ...
      ...(reddit.posts || []).slice(0, 2).map((item: any) => signal("reddit", item, item.title || item.subreddit || "Reddit discussion", item.snippet || item.title, item.sentiment, item.createdAt, item.url)),
    ...
    const critical = playNegative + freshdeskActive + n(linkedinStats.negative) + n(xStats.negative) + n(redditStats.negativeCount) + n(youtubeSentiment.negative);
    ...
    const linkedinCluster = clusterFor("linkedin"); const xCluster = clusterFor("x"); const redditCluster = clusterFor("reddit"); const youtubeCluster = clusterFor("youtube");
    const crossChannelSummary = semanticParts.length
      ? `${playCluster?.label || "App stability and loading failures"} are driving Play Store criticism. Freshdesk complaints are concentrated around ${String(freshdeskCluster?.label || "tests, admit cards and registrations").toLowerCase()}. LinkedIn is shaping professional reputation, while Reddit students are debating ${String(redditCluster?.label || "course value and academic trust").toLowerCase()} and YouTube remains teacher-led.`
      : "App experience is driving the strongest product criticism. Freshdesk complaints remain concentrated around time-sensitive learner access. LinkedIn is shaping workplace reputation, while Reddit is exposing candid course-value and academic-trust debates.";
    const narratedCards = cards.map((card) => {
      const semantic = semanticFeeds[card.id]?.summary;
      const fallback = card.summary;
    ...
    const forecasts = [
      ...
      (xCluster || linkedinCluster || redditCluster || youtubeCluster) && { eyebrow: "PUBLIC NARRATIVE", title: `${xCluster?.label || "Public reach"} will drive visibility; trust will be decided elsewhere`, text: `Watch ${String(linkedinCluster?.label || "professional reputation").toLowerCase()} on LinkedIn, ${String(redditCluster?.label || "student trust").toLowerCase()} on Reddit and ${String(youtubeCluster?.label || "teacher discussion").toLowerCase()} on YouTube.`, metric: `${n((xCluster || redditCluster)?.share).toFixed(1)}% leading share`, tone: "cool" },
    ...
    return { cards: narratedCards, insights, signals, drivers, forecasts, critical, total, topIssue, crossChannelSummary, semanticReady: semanticParts.length > 0, freshness: linkedin.generatedAt || reddit.meta?.generatedAt || playstore.livePulledAt || freshdesk.generatedAt };
  }, [feeds, semanticFeeds]);
  ...
  return <main className="audience-studio ai-overview-page">
    ...
    <header className="ai-topbar">
      ...
      <nav className="ai-source-nav" aria-label="Intelligence channels"><button className="active">Overview</button>{overviewSources.map((source) => <button key={source} onClick={() => openSource(source)}>{SOURCES[source].label}</button>)}<button onClick={() => router.replace("/shield")}>Shield</button></nav>
      <div className="ai-top-actions"><AuthProfileMenu /></div>
    </header>

    {loading ? <OvalLoadingSkeleton embedded variant="overview" /> : error ? <section className="ai-loading"><p>{error}</p><button onClick={() => location.reload()}>Retry</button></section> : <>
